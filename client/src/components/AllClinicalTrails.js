...
const AllClinicalTrials = ({ drawerOpen, setSelectedTrial }) => {
        ...
        accessorFn: (row) => (
          <Box className="action-column">
            <Typography className="action-icon delete">
              <FaFilePdf   className='action-icon download' onClick={() => handleDownload(row)}/>
              <FaEdit  className='action-icon edit' onClick={() => handleEdit(row)}/>
