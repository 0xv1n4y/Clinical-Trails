...
const MainContent = ({ drawerOpen, selectedTrial  }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (selectedTrial) {
        // If editing, send a PUT request to update the trial
        const response = await axios.put(`http://localhost:4000/api/applications/${selectedTrial.administrativeDetails.applicationId}`, formData);
        if (response.status === 200) {
          setSnackbarMessage('Form updated successfully!');
          setFormData(formDataTemplate);
        }
      } else {
        // If adding a new trial, send a POST request
